...
export default function LabourerSidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[280px] flex flex-col bg-primary z-50 overflow-y-auto">
      <div className="p-8">
        <h1 className="text-2xl font-bold text-on-surface tracking-tight">HireMe Services</h1>
        <p className="text-on-surface-variant text-xs uppercase tracking-wider mt-1">Labourer Module</p>
      </div>
      ...
      <div className="p-6 mt-auto">
        <div className="flex items-center gap-4 bg-white/10 p-4 rounded-xl border border-white/10">
          ...
          <div className="overflow-hidden">
            <p className="text-on-surface font-bold text-sm truncate">Marcus Thorne</p>
            <p className="text-on-surface-variant text-xs truncate">Senior Mason</p>
          </div>
        </div>
        <button className="flex items-center gap-2 text-on-surface-variant hover:text-on-surface text-xs mt-6 px-2 transition-colors">
          <LogOut size={14} />
          <span>Sign Out</span>
        </button>
      </div>
